...
const HAZARD_STYLES: Record<string, { bg: string; text: string; border: string }> = {
  ...
  ASH: { bg: 'bg-gray-100', text: 'text-gray-800', border: 'border-gray-400' },
};
...
function getTypeBadgeStyle(type: string) {
  return TYPE_BADGE_STYLES[type] || { bg: 'bg-gray-100', text: 'text-gray-800' };
}
              ...
              {isExpanded && (
                <div className="px-4 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-600">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-3">
                    ...
                    <div>
                      <span className="text-gray-500">Altitude range:</span>{' '}
                  ...
                  <div className="p-2 bg-gray-100 dark:bg-gray-700 rounded font-mono text-xs text-gray-700 dark:text-gray-300 break-all whitespace-pre-wrap">
                    {item.rawText}
                  </div>
                </div>
              )}
